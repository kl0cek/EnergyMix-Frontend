...
function TimeBox({ label, iso, locale }: { label: string; iso: string; locale: string }) {
  return (
    <div className="rounded-xl border border-line px-4 py-3">
      <div className="text-xs font-medium uppercase tracking-wide text-muted">{label}</div>
      <div className="tnum mt-1 text-2xl font-bold text-ink">{formatTime(iso, locale)}</div>
      <div className="mt-0.5 text-xs text-muted">{formatWeekdayDate(iso, locale)}</div>
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-3">
        <TimeBox label={t('charging.start')} iso={data.start} locale={locale} />
        <span className="text-xl text-muted">→</span>
        <TimeBox label={t('charging.end')} iso={data.end} locale={locale} />
      </div>
    </div>
  );
}
